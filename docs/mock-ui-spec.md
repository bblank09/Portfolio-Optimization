...
| Risk measure (for mean-risk objectives) | Std deviation (default) · Semi-variance · CVaR · CDaR | riskfolio-lib supports 26+; shortlist to these 4 with a real reason per measure, not just "legibility" (previously flagged as under-justified — resolved here): **Std deviation** is riskfolio-lib's own default `rm` value (confirmed from its docs/examples this session) and the standard entry point. **Semi-variance** is Markowitz's own preferred alternative to variance — he argued for it over full variance because it only penalizes downside deviation, matching investors' actual intuition of "risk" (confirmed via web search this session, corroborating a widely-cited claim). **CVaR** is confirmed via web search as the measure "consistently gaining popularity" among portfolio/risk managers and regulators over plain std deviation, due to its convexity/monotonicity properties. **CDaR** is the one measure chosen for project-specific reasons, not general popularity: the sibling backtester's own Results view already ships a full **Drawdown tab** as one of its 8 tabs, so users of this project's data/audience already have an intuitive, pre-built mental model for "drawdown" as *the* risk concept — CDaR extends that same concept into the optimizer, rather than introducing an unfamiliar risk measure from scratch |
