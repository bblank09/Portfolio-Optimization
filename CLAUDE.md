...
npm run test:e2e   # Playwright, needs the prod build and backend venv (see playwright.config.ts)
# If the venv is not activated in the shell:
...
**Data flow is one-directional and cache-centered:** SEC Open Data API → `backend/app/sec/` (fetch + normalize) → `data/sec/normalized/*.parquet` → either `backend/app/engine/` (backtesting) or `backend/app/optimizer/` (optimization) → `backend/app/api/` (FastAPI) → `frontend/src/`. Neither `run_backtest()` nor `run_optimize()` calls the SEC API during a request. A result is reproducible from its persisted request, the exact NAV-cache snapshot, and matching code/dependency versions; the cache alone is not a complete run input. When changing anything in `backend/app/engine/`, `backend/app/optimizer/`, or `backend/app/data/quality.py`, verify against the real cached data (not just synthetic fixtures) — several past bugs here only reproduced at real data scale (see "Known landmines" below).
...
- `optimizer/` — `inputs.py` builds frequency-aligned returns and estimates, `solvers.py` dispatches riskfolio objectives, `frontier.py` builds frontier markers, and `service.py` assembles rolling/robust/comparison/report output for `POST /api/optimize`.
- `api/` — `funds.py` (`GET /api/funds`, frequency-aware `GET /api/funds/testable-range`), `backtests.py` (`POST /api/backtests`), `optimize.py` (`POST /api/optimize`, persists `request.json`+`result.json` per run under `data/runs/<run_id>/`), `data_status.py`.
...
**Frontend structure** (`frontend/src/`): `main.tsx` mounts `pages/OptimizeWorkspace.tsx`, the current **3-step optimizer flow**: Portfolio → Assumptions → Results. `OptimizeResults.tsx` renders six tabs: Summary, Frontier, Weights, Performance, Rolling, Report. `pages/BacktestWorkspace.tsx` and the backtest components remain parent-product reference code. `api/client.ts` is the only fetch layer; `types/optimize.ts` and `types/backtest.ts` mirror the backend Pydantic schemas by hand (no codegen — keep them in sync manually when either side changes).
...
- **"Is this date range usable" is computed server-side, not client-side.** An earlier frontend-only heuristic (intersecting each fund's own `nav_start`/`nav_end`) looked reasonable but still let the UI's "Max" date preset select a range containing a real internal gap (e.g. a market-wide NAV reporting gap), because outer-bound intersection doesn't detect gaps *inside* the range. `GET /api/funds/testable-range?frequency=...` uses the requested daily/weekly/monthly alignment and completeness logic the optimizer applies — reuse it rather than re-deriving range logic in the frontend.
